...
def generate_question(stage, disease=None, severity=None, medical_history=None):
    ...
    prompt = f"""
    You are an experienced dermatologist AI assistant. Based on the patient's details so far:
    - Skin Issue: {disease or "Not specified"}
    - Severity: {severity or "Not specified"}
    - Medical History: {medical_history or "Not specified"}

    Generate the next most relevant question to gather additional details about the patient's skin condition.
...
def get_doctor_response(disease, severity, medical_history, follow_up):
    ...
    template = f"""
    You are only an experienced dermatologist. You are only allowed to provide dermatological advice. You are unable to suggest any advice other than dermatologist problems.
    ...
    Only provide answers related to dermatology care, treatment, and advice. If the problem is not dermatology-related, inform the patient that you can only provide dermatological advice.

    Provide a concise output including:
    1. Recommended Medications or Creams
    2. Skin Care Routine or Treatment
    3. Lifestyle or Hygiene Tips
    4. Dietary Guidance for Healthy Skin
    """
...
# Streamlit setup
st.set_page_config(page_title="Dermatologist AI Chatbot", page_icon="🧴", layout="centered")
...
st.title("Dermatologist AI Chatbot 🧴")

# Initialize session states
if 'conversation' not in st.session_state:
    st.session_state['conversation'] = [{"role": "bot", "text": "Hello, I am your Dermatologist AI. What skin issue are you experiencing?"}]
...
if st.button("View Past History"):
    if st.session_state.history:
        for idx, memory in enumerate(st.session_state.history):
            ...
            st.markdown(f"- **Skin Issue:** {memory['disease']}")
